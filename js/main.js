...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const createScrollTriggers = (selector, staggerDelay = 0.1) => {
        ...
        // Pre-load and prepare all elements in advance
        ...
        elements.forEach((element, i) => {
            ...
            const duration = isMobile ? 0.3 : 0.4; // Shorter duration for faster animation
            const delay = isMobile ? i * 0.03 : i * (staggerDelay * 0.6); // Reduced delay for quicker response
            
            ScrollTrigger.create({
                trigger: element,
                start: 'top bottom-=150', // Start trigger earlier (further from viewport)
                onEnter: () => {
            ...
            // Preload next elements when approaching viewport
            ScrollTrigger.create({
                trigger: element,
                start: 'top bottom+=200', // Even earlier pre-loading trigger
                onEnter: () => {
                    // Find the next few elements and prepare them
                    const nextElements = elements.slice(i + 1, i + 5);
                    nextElements.forEach(nextElement => {
                        // Ensure the element is ready to animate
